...
serve(async (req) => {
  ...
  if (!bill) {
    ...
    return new Response(JSON.stringify({ error: 'No bill record found' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
  ...
  if (profileError || !employeeProfile) {
    ...
    return new Response(JSON.stringify({ error: 'Employee profile not found' }), { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: 'Expense Manager <onboarding@resend.dev>', // IMPORTANT: Replace with YOUR verified Resend domain/sender (e.g., 'yourname@yourdomain.com')
      ...
      html: `
        ...
        <p>Thank you for using Expense Manager.</p>
      `,
    ...
    console.log('Employee email sent successfully:', data);

    // 5. Send ERP notification email (if configured)
    const erpEmail = Deno.env.get('ERP_EMAIL');
    if (erpEmail) {
      try {
        // Attach only the generated PDF for ERP
        const erpAttachments = [];
        if (generatedPdfUrl) {
          const generatedAttachment = await downloadFile(
            generatedPdfUrl,
            `Claim_${billId.substring(0, 6)}_${employeeUsername}.pdf`
          );
          if (generatedAttachment) erpAttachments.push(generatedAttachment);
        }

        const erpSubject = `Reimbursement Approved: ${employeeUsername} | ₹${billAmount} | ${billPurpose} | INV#${bill.invoice_no || 'N/A'}`;

        const { error: erpError } = await resend.emails.send({
          from: 'Expense Manager <onboarding@resend.dev>',
          to: [erpEmail],
          subject: erpSubject,
          html: `
            <h2>Approved Reimbursement Claim</h2>
            <table border="1" cellpadding="8" cellspacing="0" style="border-collapse: collapse;">
              <tr><td><strong>Employee</strong></td><td>${employeeUsername} (${employeeEmail})</td></tr>
              <tr><td><strong>Employee Code</strong></td><td>${employeeId.substring(0, 8)}</td></tr>
              <tr><td><strong>Purpose</strong></td><td>${billPurpose}</td></tr>
              <tr><td><strong>Amount</strong></td><td>₹${billAmount}</td></tr>
              <tr><td><strong>Bill Date</strong></td><td>${billDate}</td></tr>
              <tr><td><strong>Invoice No.</strong></td><td>${bill.invoice_no || 'N/A'}</td></tr>
              <tr><td><strong>Source</strong></td><td>${bill.source || 'N/A'}</td></tr>
              <tr><td><strong>Description</strong></td><td>${bill.description || 'N/A'}</td></tr>
              ${adminNotes ? `<tr><td><strong>Admin Notes</strong></td><td>${adminNotes}</td></tr>` : ''}
            </table>
          `,
          attachments: erpAttachments.length > 0 ? erpAttachments : undefined,
        });

        if (erpError) {
          console.error('Error sending ERP email:', erpError);
        } else {
          console.log('ERP email sent successfully to:', erpEmail);
        }
      } catch (erpException) {
        console.error('Exception sending ERP email (non-fatal):', erpException);
      }
    } else {
      console.warn('ERP_EMAIL not configured, skipping ERP notification.');
    }
